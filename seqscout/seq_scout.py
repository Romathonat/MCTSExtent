...
from general.reader import read_data_kosarak, read_data_sc2, read_jmlr, read_data
from general.bitset import compute_first_zero_mask, compute_last_ones_mask, compute_quality_vertical
from general.utils import sequence_mutable_to_immutable, k_length, \
    count_target_class_data, extract_items, compute_quality, \
    sequence_immutable_to_mutable, encode_items, encode_data, print_results_decode, print_results

from general.priorityset import PrioritySet, PrioritySetUCB
...
def compute_variations_better_quality(sequence, items, data, itemsets_memory, target_class, target_quality,
                                      enable_i=True, quality_measure=conf.QUALITY_MEASURE):
    '''
...
def exploit_arm(pattern, quality, items, data, itemsets_memory, target_class, enable_i=True,
...
def optimize_pattern(patterns, items, data, itemsets_memory, target_class, top_k, sorted_patterns, enable_i=True,
                     quality_measure=conf.QUALITY_MEASURE):
...
def seq_scout_api(dataset=conf.DATA, time_budget=conf.TIME_BUDGET, top_k=conf.TOP_K):
    ...
    results = seq_scout(data, target_class, top_k=top_k, vertical=False, time_budget=time_budget,
                        iterations_limit=10000000000000, enable_i=enable_i)
...
def launch():
    ...
    results = seq_scout(DATA, '+', time_budget=10, top_k=5, enable_i=False, vertical=False, iterations_limit=10000)
