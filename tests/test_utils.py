from general.bitset import following_ones, generate_bitset, get_support_from_vector, compute_bitset_slot_size, compute_quality_vertical
from general.utils import sequence_mutable_to_immutable, count_target_class_data, is_subsequence, create_s_extension, \
    create_i_extension, compute_quality, filter_positive, filter_empty_sequences, find_LCS
...
def test_filter_positive():
    assert len(filter_positive(data, '+')) == 1


def test_filter_empty_sequences():
    data.append(['-'])
    assert len(filter_empty_sequences(data)) == 2
